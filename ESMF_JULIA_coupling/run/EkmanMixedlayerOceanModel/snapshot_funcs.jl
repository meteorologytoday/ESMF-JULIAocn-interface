...
function loadSnapshot(
    ...
    writeLog(log_handle, "Reading files: %s", filename)
    ...
    writeLog(log_handle, "### Loaded config domain (cannot overwrite) ###")
    JSON.print(cfgs["DOMAIN"], 4)
        
    writeLog(log_handle, "### Loaded config core (overwritable) ###")
    JSON.print(cfgs["MODEL_CORE"], 4)
   
    cfg_core = cfgs["MODEL_CORE"]
    if overwrite_config != nothing
        writeLog( log_handle, "### Overwriting core config ###")
        for (k, v) in overwrite_config
            if haskey(cfg_core, k)
                if cfg_core[k] != v
                    writeLog( log_handle, "%s is overwritten: %s => %s", k,  string(cfg_core[k]), string(v))
                end
            else
                writeLog( log_handle, "%s is added: %s", k,  string(v))
            end
    ...
    writeLog( log_handle, "### Loading field from snapshot to current model ###")
    ...
    for fieldname in fieldnames(Field)
        ...
        if hasfield(typeof(loaded_fi), fieldname)
            ...
            if typeof(var) <: Array
                var.= loaded_var
                writeLog( log_handle, "Field loaded: %s", string(fieldname))
            else
                writeLog( log_handle, "Field %s is not raw array. Skip it.", string(fieldname))
            end
        else        
            writeLog( log_handle, "Field %s does not exist in snapshot. Skip it.", string(fieldname))
        end
    end
