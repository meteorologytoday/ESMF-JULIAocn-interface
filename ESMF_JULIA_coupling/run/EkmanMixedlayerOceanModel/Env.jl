mutable struct Env
    ...
    function Env(
        ...
        writeLog(log_handle, "Validating config...")
        cfgs = Dict(
            "MODEL_CORE" => validateConfigEntries(cfgs["MODEL_CORE"], getEMOMConfigDescriptors()["MODEL_CORE"]; verbose = verbose, log_handle = log_handle), 
            "DOMAIN" => validateConfigEntries(cfgs["DOMAIN"], getDomainConfigDescriptors()["DOMAIN"]; verbose = verbose, log_handle=log_handle), 
        )
